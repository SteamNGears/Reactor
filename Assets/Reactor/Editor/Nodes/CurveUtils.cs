	...
	public class CurveUtils
	{
		public static bool IsStartCurve(BaseNode n, Vector2 clickPos)
		{
			...
			if (clickPos.x > (n.position.x + n.position.width - (radius/2)) && clickPos.x < (n.position.x + n.position.width + (radius/2)))
				if ((clickPos.y > (n.position.y + (n.position.height / 2) - radius/2) && clickPos.y < ((n.position.y + (n.position.height / 2) + radius/2))))
					return true;
			return false;
		...
		{
			Vector3 startPos = new Vector3(start.x + start.width, start.position.y + start.height / 2, 0);
			Vector3 endPos = new Vector3(end.position.x, end.position.y + end.height / 2, 0);
